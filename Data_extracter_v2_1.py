...
def main(input_dir, gap, offset_x, offset_y, inner_initial, outer_initial, time_data_path):
    ...
    print(f"_Temp_processed folder for PI steady-state outputs: {save_dir}")
    ...
    try:
        ...
        for root, dirs, files in os.walk(grandparent_dir):
            csv_files = sorted([f for f in files if ('axis' in f or 'retard' in f) and f.endswith('.csv')])
            if csv_files:
                input_dir = root
    ...
    def plot_data(center_x, center_y, inner_radius, outer_radius):
        print(center_x, center_y, inner_radius, outer_radius)
        plt.ion()  # Enable interactive mode
        fig, ax = plt.subplots()
        ax.clear()
        ax.imshow(data, cmap='gray')
        ax.add_patch(Circle((center_x, center_y), inner_radius, color='r', fill=False))
        ax.add_patch(Circle((center_x, center_y), outer_radius, color='b', fill=False))
        plt.draw()
        plt.pause(0.1)
    ...
    # Initialize arrays for space-time diagrams
    space_time_circle = []
    space_time_line = []

    no_of_pi_intervals = len(csv_files)/fixed_interval
    ...
    # Trim any extra parts that are beyond the intervals
    ...
    # Verify the dimensions of the extracted slices
    print("Verifying dimensions of the extracted slices...")
    for i, (circle_slice, line_slice) in enumerate(zip(space_time_circle, space_time_line)):
        if i == 5:
            print(f"Slice {i}: Circle slice shape: {circle_slice.shape}, Line slice shape: {line_slice.shape}")
    ...
    with open(intervals_path, 'w') as f:
        f.write(str(no_of_pi_intervals))  # Save counter as a string
    ...
    print(f"PI steady-state output JSON written: {json_path}")

    # Remove the intermediate results file after processing all files
    intermediate_file = os.path.join(save_dir, 'intermediate_results.pkl')
